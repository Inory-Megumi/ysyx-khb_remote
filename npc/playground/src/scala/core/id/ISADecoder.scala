...
object ISADecoder {
  ...
  // NOP - ADDI x0, 0x00(x0) -- to be removed
...
class ISADecoder extends Module with InstConfig {
  ...
  protected val defRes   = List(nopInstType, wtRegFalse, instNOP)//default
  protected val decRes   = ListLookup(io.inst, defRes, decodeTable)//decode 
  immExten.io.inst     := io.inst
  immExten.io.instType := decRes(0)//instType
  io.isa               := decRes(2)//isaType
  io.wen               := decRes(1) //write memory
}